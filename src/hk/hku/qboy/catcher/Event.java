...
public class Event implements Cloneable {
	 ...
	 * events, endday for allday) 3) the title (unnecessary, but nice)
	 * 
	 * The start and end day is sorted first so that all day events are sorted
	 * correctly with respect to events that are >24 hours (and therefore show
	 * up in the allday area).
	 */
	...
	public static final String[] EVENT_PROJECTION = new String[] {
			...
			Instances.ALL_DAY + "=1 OR (" + Instances.END + "-"
					...
					+ " AS " + DISPLAY_AS_ALLDAY, // 19
	};
	public static final String[] TASK_PROJECTION = new String[] {
			TaskProvider._ID, // 0
			TaskProvider.TITLE, // 1
			TaskProvider.COLOR, // 2
			TaskProvider.RECORD, // 3
			TaskProvider.COMPLETED, // 4
	};
	private static final int TASK_ID_INDEX = 0;
	private static final int TASK_TITLE_INDEX = 1;
	private static final int TASK_COLOR_INDEX = 2;
	private static final int TASK_RECORD_INDEX = 3;
	private static final int TASK_COMPLETED_INDEX = 4;
	...
	public static void loadEvents(Context context, ArrayList<Event> events,
			int startDay, int days, int requestId, AtomicInteger sequenceNumber) {
		...
		try {
			...
			// boolean hideDeclined =
			// prefs.getBoolean(GeneralPreferences.KEY_HIDE_DECLINED,
			// false);
			...
			cEvents = instancesQuery(context.getContentResolver(),
					EVENT_PROJECTION, startDay, endDay, where, null,
					SORT_EVENTS_BY);
			// cAllday = instancesQuery(context.getContentResolver(),
			// EVENT_PROJECTION, startDay,
			// endDay, whereAllday, null, SORT_ALLDAY_BY);
			String selection_clause = TaskProvider.RECORD + " != ?";
			String[] selection_args = { "" };

			cAllday = instancesQuery(context.getContentResolver(),
					EVENT_PROJECTION, startDay, endDay, whereAllday, null,
					SORT_ALLDAY_BY);

			cTasks = context.getContentResolver().query(
					TaskProvider.CONTENT_URI, TASK_PROJECTION,
	...
	private static void buildTasksFromCursor(ArrayList<Event> events,
			Cursor cTasks, Context context, int startDay, int endDay) {
		if (cTasks == null || events == null) {
			Log.e(TAG, "buildTasksFromCursor: null cursor or null events list!");
			return;
		}
...
//					.getInt(PROJECTION_COLOR_INDEX));
	 ...
	 *            minimum duration acceptable as cell height of each event
	 *            rectangle in millisecond. Should be 0 when it is not
	 *            determined.
	 */
	...
	private static long removeAlldayActiveEvents(Event event,
			Iterator<Event> iter, long colMask) {
	...
	private static long removeNonAlldayActiveEvents(Event event,
			Iterator<Event> iter, long minDurationMillis, long colMask) {
